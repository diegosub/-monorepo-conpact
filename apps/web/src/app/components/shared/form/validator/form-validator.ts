...
export class FormValidations {
  ...
  static confirmaSenha(otherField: string) {
    const validator = (formControl: FormControl) => {
      if (otherField == null) {
        throw new Error('É necessário informar um campo.');
      }
      ...
      if (field.value !== formControl.value) {
        return { confirmaSenha : otherField };
      }
  ...
  static getErrorMsg(fieldName: string, validatorName: string, validatorValue?: any) {
    const config = {
      ...
      'cepInvalido': 'Este CEP é inválido.',
      'email': 'Email inválido!',
      'equalsTo': 'Os campos não são iguais',
      'confirmaSenha': 'A confirmação deve ser igual a senha',
      'pattern': 'Campo inválido',
      'mask': 'O preenchimento do campo não está correto'
    };
